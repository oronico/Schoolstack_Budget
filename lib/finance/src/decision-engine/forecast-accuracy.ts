...
// every Pursued saved scenario that has actuals captured.
//
// Lives in @workspace/finance so the planner UI (web) and the lender / board
// packet builders (api-server) share one source of truth. Both surfaces import
// `computeForecastAccuracy` and render the same numbers — keeping this in one
// place ensures the in-product view and the exported PDFs never silently
// disagree about what the same scenario means (Task #216, originally lifted
// out of `artifacts/school-financial-model/src/lib/forecast-accuracy.ts`).
//
// Types are intentionally loose (`ScenarioLike` / `ScenarioActualsLike`) so
// the api-server can call the helper with raw JSON parsed from the model
// payload without having to materialize the web's zod-inferred schemas. The
// web's richer `CustomScenario` / `CustomScenarioActuals` are structurally
// assignable, so the existing web shim re-exports this module unchanged.

import {
  computeProjectedSnapshot,
  type PersistedDecisionOverrides,
  type ProjectedSnapshot,
} from "./decision-flows.js";
import type { FullModelData, DecisionType } from "./model-shape.js";
import type { DecisionOutcomeStatus } from "../decision-bullets.js";

// Stable identifiers for the metrics we roll up. Order here drives display
// order in the UI and PDFs (headline financials first, then decision-specific
// extras).
...
// Permissive shape of a scenario's `actuals` snapshot. The web's
// `CustomScenarioActuals` (zod-inferred) carries extra fields (sourceByField,
// asOfYear validation bounds, etc.) and is structurally assignable to this.
export interface ScenarioActualsLike {
  asOfYear?: number;
  ...
  programEnrollmentActual?: number;
  notes?: string;
  updatedAt?: string;
}

// Permissive shape of a saved scenario the roll-up cares about. The web's
// `CustomScenario` (zod-inferred) is a strict superset and is structurally
// assignable to this.
...
// nothing to roll up.
export function hasComparableActuals(actuals: ScenarioActualsLike | undefined): boolean {
...
//
// Generic over T so callers (e.g. the web view passing a `CustomScenario[]`)
// keep their richer type on the returned array.
export function selectAccuracyScenarios<
  T extends { outcomeStatus?: DecisionOutcomeStatus; actuals?: ScenarioActualsLike },
>(scenarios: T[]): T[] {
...
export function computeForecastAccuracy(modelData: FullModelData): ForecastAccuracyRollup {
  const customScenarios =
    ((modelData as Record<string, unknown>).customScenarios as ScenarioLike[] | undefined) ||
    [];
...
// them ANDs the filters together. The shape mirrors what the planner UI reads
// from the URL (`?metric=…&asOfYear=…`) so the page layer can pass it through
// without translation, and the lender / board PDF routes can parse the same
// query string off the export request.
export interface ForecastAccuracyFilter {
  metric?: AccuracyMetricKey | null;
  asOfYear?: number | null;
}

// Slice an existing roll-up by metric and/or asOfYear so the UI and the
// lender / board PDFs can offer "show me only enrollment accuracy" / "only
// Year 1 actuals" cuts without re-running the (relatively expensive) per-
// scenario engine projection that `computeForecastAccuracy` performed.
// Aggregates are recomputed from the filtered entries so the headline
// tendency (e.g. "you tend to over-project X by Y%") always reflects the
// active filter rather than the unfiltered population.
//
...
//
// Originally lived in the web's `@/lib/forecast-accuracy` shim; lifted into
// `@workspace/finance` so both the planner UI and the api-server packet
// builders apply identical filter semantics (Task #391).
