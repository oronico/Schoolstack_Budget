// Integration test for GET /api/models/:id/export/loan-readiness-pdf.
//
// Sibling to tests/lender-packet-pdf-route.ts and
// tests/board-packet-pdf-route.ts. This test exists so a future refactor
// of the loan-readiness PDF route cannot silently break the audit trail
...
//   - one new row in `events` with eventName="exported_loan_readiness_pdf"
//     and metadata.modelId pointing back at the model
//   - 404 when a different user requests the model (no exports/events written)
//
// Note on flag handling (deviation from task description): the task
// description claimed loan-readiness-pdf should have a 422 path covered.
// However, the actual loan-readiness-pdf handler in routes/models.ts
// does NOT call checkUnresolvedFlags (only the lender/board packet,
// excel, and decision-comparison routes do). There is therefore no 422
// path to assert against the real handler. Following the precedent set
// by the lender/board packet test files, we assert against the real
// handler rather than the task description.
...
// The loan-readiness PDF route does not block on unresolved assumption
// flags, so we can use microschoolStartup as-is without seeding flag
// responses.
function modelData() {
...
async function getPdf(baseUrl: string, modelId: number, token: string) {
  return fetch(`${baseUrl}/api/models/${modelId}/export/loan-readiness-pdf`, {
    method: "GET",
...
async function testHappyPath(server: BootedServer) {
  ...
  const user = await createUser(`loanready-happy-${stamp}@example.com`);
  ...
  try {
    ...
    check(
      "filename is suffixed with _Loan_Readiness_Report.pdf (sanitised school name)",
      /_Loan_Readiness_Report\.pdf/.test(cd),
      `got ${JSON.stringify(cd)}`,
    );
    ...
    const eventRow = await getLatestEvent(user.id, "exported_loan_readiness_pdf");
    check("analytics event 'exported_loan_readiness_pdf' inserted", !!eventRow);
...
async function testOtherUsersModel(server: BootedServer) {
  ...
  const stamp = Date.now();
  const owner = await createUser(`loanready-owner-${stamp}@example.com`);
  const intruder = await createUser(`loanready-intruder-${stamp}@example.com`);
  ...
  try {
    ...
    const intruderEvent = await getLatestEvent(intruder.id, "exported_loan_readiness_pdf");
    check("no analytics event for intruder", !intruderEvent);
    const ownerEvent = await getLatestEvent(owner.id, "exported_loan_readiness_pdf");
...
async function main() {
  ...
  console.log("=== Loan Readiness PDF Route Integration Tests ===");
