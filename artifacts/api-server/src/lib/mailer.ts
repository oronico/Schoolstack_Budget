  ...
  const html = `
    <div style="font-family:'Nunito',Arial,sans-serif;max-width:640px;margin:0 auto;padding:32px;">
      ...
      <div style="border:1px solid #E2E8F0;border-top:none;padding:24px;border-radius:0 0 8px 8px;">
        <p style="color:#475569;line-height:1.6;margin-top:0;"><strong>${data.requesterName}</strong> (${data.requesterEmail}) has requested a review of their financial model.</p>
        ${data.message ? `<div style="background:#F8FAFC;border-radius:6px;padding:12px 16px;margin:12px 0;"><p style="color:#475569;margin:0;font-style:italic;">"${data.message}"</p></div>` : ""}
        ...
        <table style="width:100%;border-collapse:collapse;margin:8px 0 16px;">
          <tr><td style="padding:4px 0;color:#94A3B8;width:140px;">School</td><td style="color:#1E293B;font-weight:600;">${data.schoolName}</td></tr>
          <tr><td style="padding:4px 0;color:#94A3B8;">State</td><td style="color:#1E293B;">${data.state}</td></tr>
          <tr><td style="padding:4px 0;color:#94A3B8;">Type</td><td style="color:#1E293B;">${data.schoolType}</td></tr>
          <tr><td style="padding:4px 0;color:#94A3B8;">Entity</td><td style="color:#1E293B;">${data.entityType}</td></tr>
        </table>
        ...
        <div style="text-align:center;margin:24px 0 8px;">
          <a href="mailto:${data.requesterEmail}" style="background-color:#D97706;color:white;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block;">Reply to ${data.requesterName}</a>
        </div>
      </div>
...
export async function sendReviewConfirmation(toEmail: string, requesterName: string, schoolName: string): Promise<{ success: boolean; error?: string }> {
  ...
  const html = `
    <div style="font-family:'Nunito',Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px;">
      ...
      <p style="color:#475569;line-height:1.6;">
        We've received your request to review the financial model for <strong>${schoolName}</strong>. Our team will look it over and get back to you within <strong>2 business days</strong>.
      </p>
      <p style="color:#475569;line-height:1.6;">
