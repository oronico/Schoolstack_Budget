...
import { autoResolveBlockingFlags } from "./utils/auto-resolve-flags";
...
// checkUnresolvedFlags in api-server/src/routes/models.ts). Rather than
// hard-coding the resolutions per persona (which silently rots whenever
// the consultant engine adds or renames a warning rule — Task #451), we
// run the same `detectUnusualAssumptions` pass the engine would run and
// auto-seed an explanation for every blocking flag. Info-level flags do
// NOT block, so they're omitted. Adding a new persona below now requires
// only the data fixture — no companion `flagResolutions` array.
