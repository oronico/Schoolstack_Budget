...
export interface MicroLesson {
  ...
  // Title of the wizard step this lesson should fire on (e.g. "Expenses",
  // "Staffing"). Resolved against the current visible step list at render
  // time so the lesson lands on the correctly-named step regardless of
  // model duration (single vs five-year) or school-type variant
  // (Chesterton inserts extra steps and renumbers the rail). A title that
  // doesn't exist in the current visible step list is silently skipped —
  // never coerced onto a wrong-but-existing step.
...
export const MICRO_LESSONS: MicroLesson[] = [
  ...
  },
  {
    ...
    readTimeSeconds: 25,
    triggerStepTitle: "Assumptions & Sensitivity",
    checkTrigger: (data) => {
...
// Returns lessons whose `triggerStepTitle` matches the wizard's currently
// visible step title. Matching on title (rather than positional step ID)
// means a lesson tied to "Expenses" lands on the Expenses step regardless
// of model duration (single vs five-year) or school-type variant
// (Chesterton inserts Fundraising/Gift Chart/Recruiting between Staffing
// and Expenses, which would shift Expenses from step 6 to step 9 under
// the old positional scheme). A `triggerStepTitle` that isn't currently
// visible — e.g. a lesson tied to "Lender Narrative" while the founder
// is in single-year mode that hides that step — simply never matches
// (currentStepTitle will never equal it), so the lesson is safely
// skipped instead of firing on a wrong-but-existing step.
export function getTriggeredLessons(
  data: FullModelData,
  ...
  return MICRO_LESSONS.filter(
    (lesson) =>
      lesson.triggerStepTitle === currentStepTitle &&
