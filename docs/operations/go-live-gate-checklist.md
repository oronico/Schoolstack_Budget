...
| # | Gate | Evidence | State |
|---|---|---|---|
| 2.1 | `api-tests` workflow on `main` HEAD reads green end-to-end. | Workflow status badge or last full-chain log in `/tmp/logs/api-tests_*.log`. The chain runs `pnpm --filter @workspace/api-server run test` — currently 100+ test scripts including the math-integrity harness. | [!] — Gated on resolution of the full **#985 → #984 → #994 → #995 → #981** chain. Critical path, in order: **#985** (current proximate blocker — in-chain `test:parity` state pollution; suite passes 142/142 in isolation but fails first-in-chain with DSCR/Cash diffs against charter/chesterton/heritage-private goldens) → **#984** (cross-engine consultant-vs-underwriting drift root cause that #985 partially surfaces) → **#994** (consultant-engine ↔ underwriting reconciliation work that closes #984) → **#995** (canonical-baseline refresh after the reconciliation lands so the harness has correct goldens to compare against) → **#981** (final math-integrity harness re-baselining and CI re-greening). This whole chain MUST close before this gate opens — fixing #985 alone is necessary but not sufficient. |
...
| 2.3 | The component-state extractor coverage covers both `extractComponentState` (props-walk superset, in `artifacts/api-server/src/lib/integrity/extract/component-state.ts`) AND `extractRendered` (DOM-walk subset, in `artifacts/school-financial-model/src/lib/integrity/extract-rendered.ts`). The split is intentional: the props-walk catches shape/wire bugs but a superset of what is rendered; the DOM-walk is a subset (only what the founder actually sees). M5 consumes both as named, separate surfaces so render-time conditional bugs (e.g. cap-callout suppression) are caught. | `rg -l "extractComponentState\|extractRendered" artifacts/api-server/tests/math-integrity-harness.ts` returns the harness file. Paste both extractor import lines from that file into this cell. | [ ] |
...
| 9.3 | Pre-cutover affected-record counts have been run against the prod read-replica and inserted into the M6 plan's "Affected records" column. (M6 ships the SQL; M7 runs it.) | **Dependency: prod read-replica credential.** Run the M6 plan's `SELECT count(*)` snippets and write the counts back in. Track the access request as a first-class sub-task here, not a footnote: (i) **Owner of the access request:** _(name the engineer who has or is filing it — TBD; assign by **May 22 EOD**, well before the May 29 review)_. (ii) **Request status:** _(not-yet-filed / in-flight / granted — update inline)_. (iii) **Expected grant date:** _(record once filed; if the gap to May 29 is < 48 h treat as unmet)_. (iv) **Fallback path if access does NOT land ≥ 48 h before cutover** — pick one, do NOT decide under pressure on the day: (a) **delay cutover** by one calendar week and refile with executive escalation [default]; (b) **partial waiver** — generate the counts against the staging dataset, annotate every cell as "staging-estimate, not prod" in the M6 plan, AND obtain a co-signed waiver from the reviewer + engineering captured in §D of the M7 summary; (c) **defer** the count to the cutover window itself with pre-staged rollback (only acceptable if §9.2 rollback for every migration is rehearsed and timed). Once counts are run, paste the run date + the M6 plan commit SHA. | [ ] |
...
| # | Gate | Evidence | State |
|---|---|---|---|
| 10.1 | Review **confirmed** (not "tentatively held") with Lance Helming for **Fri May 29, 2026** as a **≥ 3-hour contiguous block** — CFO calendars do not have casual half-day slots, lock the invite **on or before Fri May 22, 2026 EOD**, not later. The block covers: (i) end-to-end persona walkthrough (wizard input → engine compute → workbook export → packet PDF → lender-facing presentation), (ii) live cross-reference of each engine number the reviewer questions to its specific M4 report row, (iii) disposition decision on every blocking finding (fix-same-day / waive / defer), and (iv) sign-off of §E of the M7 completion summary. **Backup escalation:** if Lance has not returned a calendar confirmation by **Fri May 22, 2026 EOD**, route the same invite to Allison the same day and re-confirm with her by **Tue May 26, 2026 EOD**. Do not wait past May 22 hoping Lance will reply — the Allison path needs four business days of lead time too. | Calendar invite confirmation (accepted, not tentative) covering ≥ 3 hours on the named date; paste the confirmation date + reviewer name here. If routed to Allison, paste both the May 22 unanswered-by-Lance marker AND Allison's confirmation date. | [ ] |
